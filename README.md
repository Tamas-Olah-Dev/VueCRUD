...
```
This allows for generating index links to models. Its first parameter can contain field-value pairs to be used as default filters (so we can use ['email' => '@datalytix.tech'] to set this as a default search for the email field, if it's defined on the model).     
